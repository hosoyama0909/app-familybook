...
| STK-02 | SYR-10 | SWR-LOG-09 | LogView | datetime-local編集（日付＋時刻） | `.ltedit` onchange→ts更新 | QTC-EDIT-01, QTC-DATE-01 |
| STK-02 | SYR-10 | SWR-LOG-10 | LogView | メモinline編集 | `.lmemoedit` onchange | QTC-MEMO-01 |
| STK-02 | SYR-10,13 | SWR-LOG-11 | LogView | childId付与/編集 | `#lgChild`/`.lchild`, `childOptions()` | QTC-WHO-01/02 |
| STK-03 | SYR-16 | SWR-LOG-12 | StoryService/RecordView | childName反映 | tripFactsText/renderRecap/buildBooklet | QTC-WHO-01(prompt) |
| STK-06,08 | SYR-23 | SWR-UNDO-01 | DataStore(UndoStack) | snap/doUndo/toastU | `snap()`+`#toastUndo` | QTC-UNDO-01/02 |
| STK-01 | SYR-02,23 | SWR-SCHED-01 | SchedView | 行タップ編集 | `.evtap`/evEdit/`#evCancel` | QTC-EVEDIT-01/02 |
| STK-06 | SYR-N4/04/20/02 | SWR-UI-01〜04 | Router/PackView/Share/Sched | 各小改善 | go()autoscroll/pkcount/download名/今日バッジ | QTC-PACKCNT-01他 |
...
| STK-03 | SYR-16 | SWR-AWARD-01 | RecordView | tripAwards | `tripAwards()`（AI不要集計） | QTC-AWARD-01 |

## 3.7 縦のトレース：子どもへの手紙 ＋ Stop 4（学習・提案）

| STK | SYR | SWR | SWA(コンポーネント) | SWD | 実装 | テスト |
|-----|-----|-----|--------------------|-----|------|--------|
| STK-03 | SYR-17 | SWR-LETTER-01 | StoryService | generateLetter | `generateLetter()`/`trip.letters` | QTC-LETTER-01/02 |
| STK-03 | SYR-17 | SWR-LETTER-02 | StoryService | buildLetterPrompt | `buildLetterPrompt()` | QTC-LETTER-01 |
| STK-03 | SYR-16,17 | SWR-WORKER-02 | StoryWorker | Worker汎用化 | `payload.prompt` 優先 | QTC-REG-04 |
| STK-05 | SYR-18 | SWR-HP-01 | InsightsEngine | tripHP | `tripHP()` | QTC-HP-01 |
| STK-05 | SYR-19 | SWR-LEARN-01 | InsightsEngine | learnInsights | `learnInsights()`/`renderHomeInsights()` | QTC-LEARN-01 |
| STK-04,05 | SYR-21 | SWR-OPEN-01 | InsightsEngine/AtlasView | openingSuggestion | `openingSuggestion()` | QTC-OPEN-01 |
| STK-03 | SYR-22 | SWR-BOOK-01 | RecordView(BookletBuilder) | buildBooklet/printBooklet | `buildBooklet()`＋@media print | QTC-BOOK-01 |
...
## 5. カバレッジ・サマリー
- STK 8件中、Stop 0–4 で **STK-01,02,03,04,05,06,07** をカバー。STK-08 は全体で継続。
- 全 SWR-LOG/CHILD/REC/PREF/STORY/AWARD/LETTER/HP/LEARN/OPEN-* が **実装＋テストに到達**（孤立要求なし）。
